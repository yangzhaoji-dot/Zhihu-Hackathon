...
const SLIDES: LawSlide[] = [
  {
    ...
    lawCopy:
      "这颗星球保存着一条来自旧人类时代的判断。它谈的并不只是“累了就走”，而是更古老也更难回答的问题：一个原本能够休息、恢复、重新回到正常节奏的人，会不会有一天失去这种能力。",
    reality:
      "为了读懂这句话，我们先不急着翻开旧人类留下的记录。先看支配这颗星球的法则——动力系统中的鞍结分岔。它讲述的，恰好是一种“原本存在的稳定，如何走到尽头”。",
    tone: "climax",
    diagram: "overview",
  },
  {
    ...
    math: "ẋ = r + x²",
    lawCopy:
      "这是一条真实的动力系统方程。ẋ 表示状态 x 随时间变化的速度；当 ẋ = 0，意味着系统停在一个平衡状态。对 ẋ = r + x² 来说，只要 r < 0，就存在两个平衡点 x = ±√(-r)：其中一个稳定，另一个不稳定。随着 r 缓慢增大，这两个平衡点会逐渐靠近；当 r = 0，它们在同一点相遇；r 再继续增大，方程 r + x² = 0 便不再拥有实数平衡解。两个平衡态相遇并一起消失，这就是鞍结分岔。",
    reality:
      "这颗星球借走的，不是方程里的某个字母，而是它的命运：一个状态可以长久存在，也可以在外部条件持续变化时一步步逼近边界，最后不是“变得更差”，而是失去继续存在的条件。",
    tone: "law",
    diagram: "overview",
  },
  {
    ...
    reality:
      "旧人类世界里，人们有一个很朴素的说法：还能缓过来。一次熬夜以后睡两晚，精神会回来；一个项目冲刺结束，过完周末还能重新集中注意力。疲惫已经出现，但“休息以后还能回到原来的自己”这件事仍然成立。",
    tone: "law",
    diagram: "stable",
  },
  {
    ...
    reality:
      "在那些失落的人类记录里，这种变化很少以灾难的样子出现。它更像是：以前睡一晚就能缓过来，后来需要两三天；以前一个周末足够，后来到了周一仍然疲惫；以前请一次假可以重整状态，后来只能短暂减轻。人仍然上班、开会、交付，只有“恢复所需要的代价”在悄悄变大。",
    tone: "law",
    diagram: "approach",
  },
  {
    ...
    reality:
      "旧人类并不会在某一天收到“你已到达临界点”的通知。它可能只是某次休假之后，第一次发现自己没有真正恢复；也可能是重新回到原来的工作节奏时，失眠、心悸或强烈焦虑立刻卷土重来。真正改变的，不是某一天突然更累，而是“休息以后还能回来”这个前提开始失去支撑。",
    tone: "climax",
    diagram: "critical",
  },
  {
    ...
    reality:
      "这也是这颗星球所说的“止损”真正开始获得重量的时刻。旧人类留下过这样的经验：反复休息仍无法恢复，而离开原来的环境之后，睡眠、情绪或身体反应才明显缓解。此时需要重新判断的，不再只是“今天还能不能撑住”，而是——继续留在这里，同时保持过去那个自己，是否仍然可能长期成立。",
    tone: "climax",
    diagram: "gone",
  },
  {
    label: "LAW INFERENCE · 法则推论",
    title: "于是，这颗星球留下了那句旧人类判断。",
    lawCopy:
      "“长期消耗身心的工作，离开也可以是一种止损。”鞍结分岔并没有替任何人作出选择。它只是把这句话照得更清楚：长期消耗真正值得警惕的，不只是损耗在增加，而是恢复机制本身可能正在失去存在条件。",
    reality:
      "因此，离开有时并不是放弃一个仍然稳定的世界。它可能只是承认：旧的平衡已经无法继续维持。法则能把这个结构揭示出来，但它仍然不知道，人类究竟是怎样一步步走到这里的。",
    tone: "climax",
    diagram: "gone",
  },
  {
    label: "ARCHIVE SIGNAL · 失落人类回声",
    title: "法则沉默以后，旧人类的声音开始抵达。",
    lawCopy:
      "在遥远的人类时代，有人经历过加班、失眠、心悸、恢复、离开；也有人发现，离开并没有解决全部问题。他们把这些经历写进一个名为“知乎”的公共知识档案。后来，文字散落了下来，像围绕这颗星球运行的微弱信号。",
    reality:
      "下一层不再继续推演数学。我们沿着这些信号进入失落的人类档案：去看是谁写下了这些话，他们究竟经历了什么，哪些记录与这条法则产生回响，又有哪些记录提醒我们——任何一条法则，都无法囊括一个完整的人类世界。",
    diagram: "records",
  },
];
  ...
  if (stage === "records") {
    return (
      <div className={styles.recordsVisual} aria-hidden>
        ...
        <div className={`${styles.recordCard} ${styles.recordOne}`}><i />H-001</div>
        <div className={`${styles.recordCard} ${styles.recordTwo}`}><i />H-002</div>
        <div className={`${styles.recordCard} ${styles.recordThree}`}><i />H-003</div>
        <div className={styles.recordCore}>HUMAN ARCHIVE</div>
      </div>
    );
  }
