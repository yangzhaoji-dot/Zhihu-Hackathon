...
export function PlanetArrivalGuide({ opinionId }: { opinionId: string }) {
  ...
  const response = choice === "hypothesis"
    ? (zh
        ? "先把这个猜想留着。别急着把它当答案——去找一处发出微光、看起来不像背景装饰的载体，看看材料会不会支持你。"
        : "Keep that hypothesis for now. Do not turn it into an answer yet—look for a faintly glowing carrier that does not belong to the background and see what the material supports.")
    : (zh
        ? "好。先看环境，不看结论。地面很淡的光路和远处缓慢呼吸的微光都是真实的探索线索，挑一个方向走过去。"
        : "Good. Read the environment before the conclusion. The faint routes on the ground and softly pulsing lights in the distance are real exploration cues—choose one and walk toward it.");
