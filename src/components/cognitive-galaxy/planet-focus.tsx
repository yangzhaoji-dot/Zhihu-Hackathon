...
export function PlanetFocus({ node, galaxy, onClose }: { node: GalaxyNode; galaxy: Galaxy; onClose: () => void }) {
  ...
  return <motion.aside className={styles.focus} aria-label={t("focusSummary")} data-el="planet-focus" initial={{ opacity:0, y:reduced ? 0 : 12 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} transition={{ duration:reduced ? 0 : .35 }}>
    ...
    {!landed ? <button type="button" className={styles.landing} onClick={() => setLanded(true)} data-el="land-planet">{t("landing")}<ArrowRight size={16}/></button> : <div className={styles.endNote} role="status"><h3>{t("landingSoon")}</h3><p>{t("landingDetail")}</p><button type="button" onClick={onClose}>{t("backToCluster")} →</button></div>}
  </motion.aside>;
}
