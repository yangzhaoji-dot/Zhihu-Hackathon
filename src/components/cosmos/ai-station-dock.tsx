...
export function AiStationDock() {
  ...
  const surfaceSignal = station.summary?.trim() || (zh
    ? "这个节点由 AI 根据当前星系中的真人观点关系生成，用来帮助寻知者导航与综合。"
    : "This node is generated by AI from relationships between human opinions in the current galaxy to support navigation and synthesis.");
  const deepSynthesis = station.reason?.trim() || station.summary?.trim() || (zh
    ? "当前没有更多可以可靠展开的综合内容。"
    : "No deeper synthesis can be reliably expanded yet.");
    ...
    >
      ...
      <section className={styles.dock}>
        ...
        <div className={styles.section}>
          <span>{zh ? "站点信号" : "STATION SIGNAL"}</span>
          <p>{surfaceSignal}</p>
        </div>
        ...
        <div className={styles.crossRead} data-unlocked={crossReadingUnlocked ? "true" : "false"}>
          ...
          {crossReadingUnlocked ? (
            <>
              <p>
                {zh
                  ? "已建立至少两条真正理解过的认知航线。下面这段内容可以作为跨观点线索来读，但仍然不是新的真人观点。"
                  : "At least two understood cognition routes are connected. The synthesis below can now be read as a cross-opinion clue, but it is still not a new human opinion."}
              </p>
              <blockquote className={styles.deepSynthesis}>{deepSynthesis}</blockquote>
            </>
          ) : (
            <p>
              {zh
                ? "先去共鸣至少两颗与这里相连的真人观点星球。中转站不会在你真正读过材料之前提前展开深层综合。"
                : "Resonate with at least two connected human-opinion planets first. The station will not reveal deeper synthesis before you have actually read the material."}
            </p>
          )}
        </div>

        {crossReadingUnlocked && station.conditions?.length ? (
